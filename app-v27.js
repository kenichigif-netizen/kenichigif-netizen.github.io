console.log("fish-memo v27 - EXIF date time GPS autofill");
...
const isFishNoteAndroid =
  navigator.userAgent.includes("FishNoteAndroid/");

const androidAuthLock = async (
  _name,
  _acquireTimeout,
  fn
) => fn();
...
const FISH_IDENTIFY_FUNCTION_NAME = "fish-identify";
...
const aiFishSection = document.getElementById("aiFishSection");
const aiFishButton = document.getElementById("aiFishButton");
const aiFishMessage = document.getElementById("aiFishMessage");
const aiFishResult = document.getElementById("aiFishResult");
const aiFishOverallNote = document.getElementById("aiFishOverallNote");
const aiFishCandidates = document.getElementById("aiFishCandidates");
...
const detailEditRecordButton = document.getElementById(
  "detailEditRecordButton"
);
...
let aiFishRunning = false;
let aiFishHasResults = false;
...
async function editCurrentFishName() {
  ...
  try {
    ...
    if (!Array.isArray(updatedRows) || updatedRows.length === 0) {
      throw new Error(
        "更新対象のレコードを変更できませんでした。ログインユーザーまたはRLS設定を確認してください。"
      );
    }
...
async function initializeApp() {
  /*
   * v22:
   * 起動時の認証確認が止まっても白画面にしない。
   * まずログイン画面を表示し、その後保存済みセッションを確認する。
   */
  loginPanel.classList.remove("hidden");
  mainApp.classList.add("hidden");
  setMessage(
  ...
  try {
    ...
    console.error("startup auth error", error);
    setLoginMode();
    setMessage(
      loginMessage,
      `接続を確認できませんでした：${
...
function isUnknownFishName(value) {
  const name = String(value || "").trim();
  return !name || name === "不明魚";
}

function hasAiUsablePhoto(record = editingRecord) {
  return (
    selectedImageBlobs.length > 0 ||
    Boolean(record?.image_path) ||
    Boolean(record?.image_path_2)
  );
}

function clearAiFishResult() {
  aiFishHasResults = false;
  aiFishCandidates.innerHTML = "";
  aiFishOverallNote.textContent = "";
  aiFishResult.classList.add("hidden");
  clearMessage(aiFishMessage);
}

function updateAiFishSectionVisibility() {
  const unknown =
    isUnknownFishName(fishInput.value) ||
    editingRecord?.fish_name === "不明魚";

  const shouldShow =
    unknown &&
    (hasAiUsablePhoto(editingRecord) || aiFishHasResults);

  aiFishSection.classList.toggle("hidden", !shouldShow);

  if (!shouldShow && !aiFishRunning) {
    clearAiFishResult();
  }
}
...
function blobToBase64(blob) {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => {
      reject(new Error("写真をAI判定用に読み込めませんでした。"));
    };
...
function normalizeAiCandidate(candidate) {
  const fishName = String(candidate?.fishName || "").trim();

  if (!fishName || fishName === "不明魚") {
    return null;
  }
...
function renderAiFishCandidates(result) {
  aiFishCandidates.innerHTML = "";
  ...
  aiFishOverallNote.textContent =
    String(result?.overallNote || "").trim() ||
    "AIによる画像判定結果です。似た魚種がいる場合があるため、特徴を確認してください。";

  aiFishResult.classList.remove("hidden");
  aiFishHasResults = true;

  if (!result?.isFish || candidates.length === 0) {
    const noResult = document.createElement("div");
    noResult.className = "ai-fish-candidate-reason";
    noResult.textContent =
      "魚種を特定できませんでした。別角度の写真があれば追加して、もう一度試してください。";
    aiFishCandidates.appendChild(noResult);
    updateAiFishSectionVisibility();
    return;
  }

  candidates.forEach((candidate, index) => {
    const row = document.createElement("div");
    row.className = "ai-fish-candidate";

    const main = document.createElement("div");
    main.className = "ai-fish-candidate-main";

    const name = document.createElement("div");
    name.className = "ai-fish-candidate-name";
    name.textContent = `第${index + 1}候補：${candidate.fishName}`;

    const reason = document.createElement("div");
    reason.className = "ai-fish-candidate-reason";
    reason.textContent =
      candidate.reason || "写真の外見的特徴から推定しました。";

    main.appendChild(name);
    main.appendChild(reason);

    const confidence = document.createElement("div");
    confidence.className = "ai-fish-confidence";
    confidence.textContent = candidate.confidence === null
      ? "AI候補"
      : `AI目安 ${candidate.confidence}%`;

    const useButton = document.createElement("button");
    useButton.type = "button";
    useButton.className = "ai-fish-use-button";
    useButton.textContent = `${candidate.fishName} を魚種欄に入れる`;
    useButton.addEventListener("click", () => {
      fishInput.value = candidate.fishName;
      setMessage(
        aiFishMessage,
        `${candidate.fishName} を魚種欄に入力しました。確認して「${
          editingRecord ? "変更を保存" : "保存する"
        }」を押してください。`,
        "success"
      );
      fishInput.scrollIntoView({
        behavior: "smooth",
        block: "center"
      });
    });
    ...
    aiFishCandidates.appendChild(row);
  });

  updateAiFishSectionVisibility();
}

async function getAiImagePayloads(record = editingRecord) {
  const images = [];

  if (selectedImageBlobs.length > 0) {
    for (const blob of selectedImageBlobs.slice(0, 2)) {
      images.push({
...
function normalizeExistingFishIdentifyResult(data) {
  const primaryName =
    String(data?.fishName || "").trim();
  ...
  oldCandidates.forEach((item) => {
    addCandidate(
      item?.name || item?.fishName,
      item?.confidence,
      item?.reason || ""
    );
  });
  ...
  return {
    ...
    overallNote:
      overallReason ||
      "Geminiが写真の特徴から魚種候補を推定しました。",
    candidates: normalized.slice(0, 3)
  };
}

async function identifyFishWithAi(record = editingRecord) {
  if (aiFishRunning) {
    return;
  }
  ...
  if (!hasAiUsablePhoto(record)) {
    setMessage(
      aiFishMessage,
      "AI判定には写真が必要です。先に写真を追加してください。",
      "error"
    );
    photoInput.focus();
    return;
  ...
  aiFishRunning = true;
  aiFishButton.disabled = true;
  aiFishButton.textContent = "AI判定中…";
  clearMessage(aiFishMessage);

  setMessage(
    aiFishMessage,
    "Geminiが写真から魚種を判定しています…",
    "info"
  );

  try {
    const images =
      await getAiImagePayloads(record);

    if (images.length === 0) {
      throw new Error(
        "AI判定に使用できる写真を読み込めませんでした。"
      );
    }
    ...
    const response = await withTimeout(
      fetch(
        `${SUPABASE_URL}/functions/v1/${FISH_IDENTIFY_FUNCTION_NAME}`,
        {
          method: "POST",
          headers: {
            Authorization:
    ...
    if (!response.ok) {
      throw new Error(
        data?.error ||
        data?.message ||
        `AI判定サービスからエラーが返されました（${response.status}）`
      );
    }

    const normalized =
      normalizeExistingFishIdentifyResult(data);

    renderAiFishCandidates(normalized);

    setMessage(
      aiFishMessage,
      "AI判定が完了しました。候補から選んでください。",
      "success"
    );
  } catch (error) {
    console.error(
      "AI fish identification error",
      error
    );
    ...
    aiFishRunning = false;
    aiFishButton.disabled = false;
    aiFishButton.textContent =
      "AIで魚種を判定";
  }
}

function openEditModalAndIdentify(record) {
  openEditModal(record);

  if (!hasAiUsablePhoto(record)) {
    aiFishSection.classList.remove("hidden");
    setMessage(
      aiFishMessage,
      "AI判定には写真が必要です。先に写真を追加してください。",
      "info"
...
function getExistingPhotoCount(record) {
  if (!record) {
    return 0;
  }

  return Number(Boolean(record.image_path)) +
    Number(Boolean(record.image_path_2));
}

function getAvailablePhotoSlots() {
  return Math.max(0, 2 - getExistingPhotoCount(editingRecord));
}
...
function renderEditExistingPhotos() {
  ...
  if (editingRecord.signedUrl) {
    previewImage1.src = editingRecord.signedUrl;
  }

  if (editingRecord.signedUrl2) {
    previewImage2.src = editingRecord.signedUrl2;
  }

  if (editingRecord.signedUrl || editingRecord.signedUrl2) {
    preview.style.display = "grid";
  }
}

function renderEditExistingVideo() {
  if (!editingRecord) {
    return;
  }
  ...
  if (editingRecord.videoSignedUrl) {
    videoPreview.src = editingRecord.videoSignedUrl;
    videoPreviewMeta.textContent = Number.isFinite(
      Number(editingRecord.video_duration_seconds)
    )
      ? `登録済み動画 ・ ${formatDuration(
          Number(editingRecord.video_duration_seconds)
        )}`
      : "登録済み動画";
...
function renderEditExistingMedia() {
  ...
  renderEditExistingVideo();
}

function updateEditMediaNotes() {
  if (!editingRecord) {
    photoInput.disabled = false;
    photoNote.textContent =
      "JPEG・PNG・HEICなど。2枚まで選べます。写真なしでも保存できます。";
    videoNote.textContent =
      "MP4・MOV・WebMに対応。3分を超える動画は保存できません。";
    return;
  }

  const existingPhotoCount = getExistingPhotoCount(editingRecord);
  const availableSlots = getAvailablePhotoSlots();

  photoInput.disabled = availableSlots === 0;
  photoNote.textContent = availableSlots > 0
    ? `現在写真${existingPhotoCount}枚。あと${availableSlots}枚追加できます。`
    : "写真はすでに2枚登録されています。";

  videoNote.textContent = editingRecord.video_path
    ? "動画は登録済みです。新しい動画を選ぶと差し替えます。"
    : "MP4・MOV・WebMに対応。3分以内の動画を追加できます。";
}
...
function openEditModal(record) {
  ...
  addModalTitle.textContent = "釣果を編集";
  saveButton.textContent = "変更を保存";

  fishInput.value = record.fish_name === "不明魚"
    ? ""
    : (record.fish_name || "");

  const recordDate = getRecordDateValue(record);
  caughtAtInput.value = recordDate
    ? formatDateTimeLocalValue(new Date(recordDate))
    : "";

  locationInput.value = record.location_name || "";
  ...
  if (
    ...
    locationResolvedFor = record.location_name || "";
  }

  renderEditExistingMedia();
  updateEditMediaNotes();
  clearAiFishResult();
  updateAiFishSectionVisibility();
  clearMessage(addMessage);
  clearMessage(locationMessage);
  ...
  showModal(addModal);
  requestAnimationFrame(() => fishInput.focus());
}
...
function resetAddForm() {
  editingRecord = null;
  addModalTitle.textContent = "釣果追加";
  ...
  aiFishRunning = false;
  aiFishButton.disabled = false;
  aiFishButton.textContent = "AIで魚種を判定";
  updateAiFishSectionVisibility();
...
function clearExifInfo() {
  exifInfo.textContent = "";
  exifInfo.classList.add("hidden");
}

function showExifInfo(parts) {
  const items = (parts || []).filter(Boolean);
  if (!items.length) {
    clearExifInfo();
    return;
  }
  exifInfo.textContent = `写真情報：${items.join(" / ")}`;
...
async function readPhotoExif(file) {
  if (!window.exifr?.parse) return null;
  ...
  try {
    ...
    const date =
      normalizeExifDate(meta.DateTimeOriginal) ||
      normalizeExifDate(meta.CreateDate) ||
      normalizeExifDate(meta.DateTimeDigitized) ||
      normalizeExifDate(meta.ModifyDate);
...
function mergeExifMetadata(items) {
  const out = { date: null, latitude: null, longitude: null, make: "", model: "" };

  for (const item of items || []) {
    if (!item) continue;
    if (!out.date && item.date) out.date = item.date;
    ...
    if (!out.make && item.make) out.make = item.make;
    if (!out.model && item.model) out.model = item.model;
  }

  return out;
}

function getConciseReverseLocation(result) {
  if (!result) return "";
  ...
  const place =
    ...
    String(
      a.harbour || a.marina || a.pier || a.amenity ||
      a.leisure || a.road || a.neighbourhood || a.suburb || ""
    ).trim();
...
async function reverseGeocodeLocation(lat, lng) {
  ...
  if (!response.ok) {
    throw new Error(`位置情報の住所変換エラー（${response.status}）`);
  }

  return response.json();
}

function showExifLocationPin(lat, lng) {
  selectedLocation = { lat, lng };

  try {
    locationMapSection.classList.remove("hidden");
    ensureLocationMap([lat, lng]);
    ...
    console.warn("EXIF GPS map display failed", error);
  }
}

async function applyExifMetadata(meta) {
  if (!meta) {
    clearExifInfo();
    return;
  }
  ...
  if (meta.date) {
    info.push(`撮影日時 ${meta.date.toLocaleString("ja-JP")}`);

    // 手入力や既存データは上書きしない
    if (!caughtAtInput.value) {
      caughtAtInput.value = formatDateTimeLocalValue(meta.date);
      dateApplied = true;
    }
  }
  ...
  if (hasGps) {
    ...
    if (!locationInput.value.trim() && !selectedLocation) {
      ...
      showExifLocationPin(lat, lng);
      gpsApplied = true;

      try {
        const result = await reverseGeocodeLocation(lat, lng);
        const locationName = getConciseReverseLocation(result);
  ...
  if (camera) info.push(`撮影機器 ${camera}`);
  if (dateApplied) info.push("日時を自動入力");
  if (gpsApplied) info.push("場所を自動入力");
...
async function handlePhotoChange() {
  ...
  const maxNewPhotos = editingRecord
  ...
  try {
    // EXIFは縮小前の原本から読み取る
    const exifItems = await Promise.all(
      files.map((file) => readPhotoExif(file))
    );
    await applyExifMetadata(mergeExifMetadata(exifItems));
    ...
    if (editingRecord?.signedUrl) {
      previewSources.push(editingRecord.signedUrl);
    }

    if (editingRecord?.signedUrl2) {
      previewSources.push(editingRecord.signedUrl2);
    }

    for (const blob of selectedImageBlobs) {
      const url = URL.createObjectURL(blob);
      previewObjectUrls.push(url);
      previewSources.push(url);
    }

    [previewImage1, previewImage2].forEach((img, index) => {
      if (previewSources[index]) {
        img.src = previewSources[index];
      } else {
        img.removeAttribute("src");
      }
    });

    preview.style.display = previewSources.length > 0
      ? "grid"
      : "none";
    ...
    setMessage(
      addMessage,
      `写真${selectedImageBlobs.length}枚を追加できます。EXIFがあれば撮影日時・GPSを自動反映します。`,
      "success"
    );
  } catch (error) {
    console.error(error);
...
async function handleVideoChange() {
  ...
  try {
    ...
    setMessage(
      addMessage,
      editingRecord?.video_path
        ? `新しい動画に差し替えます（${formatDuration(duration)}）。`
...
async function saveFish() {
  ...
  const maxNewPhotos = recordBeingEdited
    ? Math.max(0, 2 - getExistingPhotoCount(recordBeingEdited))
    : 2;

  if (selectedImageBlobs.length > maxNewPhotos) {
    setMessage(
      addMessage,
      recordBeingEdited
        ? `追加できる写真はあと${maxNewPhotos}枚です。`
  ...
  let imagePath = recordBeingEdited?.image_path || null;
  let imagePath2 = recordBeingEdited?.image_path_2 || null;
  let videoPath = recordBeingEdited?.video_path || null;
  let videoMimeType = recordBeingEdited?.video_mime_type || null;
  let videoDurationSeconds =
    recordBeingEdited?.video_duration_seconds ?? null;

  try {
    const photoSlots = [imagePath, imagePath2];

    for (const blob of selectedImageBlobs) {
      const emptyIndex = photoSlots.findIndex((path) => !path);

      if (emptyIndex < 0) {
        throw new Error("写真は2枚まで保存できます。");
      }

      const newPath = `${currentUser.id}/${makeStorageFileName()}`;
      ...
      photoSlots[emptyIndex] = newPath;
    }

    [imagePath, imagePath2] = photoSlots;
    ...
    if (selectedVideoFile) {
      ...
      const { error } = await supabaseClient.storage
        .from("fish-photos")
        .upload(newVideoPath, selectedVideoFile, {
      ...
      if (recordBeingEdited?.video_path) {
        replacedOldPaths.push(recordBeingEdited.video_path);
      }

      videoPath = newVideoPath;
      videoMimeType = getVideoContentType(selectedVideoFile);
    ...
    if (recordBeingEdited) {
      /*
       * v25:
       * RLSに所有者判定を任せ、レコードIDで更新します。
       * さらに更新結果を取得し、0件更新なのに成功表示する問題を防ぎます。
       */
      const {
        data: updatedRows,
        error: updateError
    ...
    if (replacedOldPaths.length > 0) {
      const { error: cleanupOldError } =
        await supabaseClient.storage
          .from("fish-photos")
          .remove(replacedOldPaths);

      if (cleanupOldError) {
        console.warn(
          "差し替え前の動画ファイルを削除できませんでした。",
          cleanupOldError
        );
      }
    }

    const wasEditing = Boolean(recordBeingEdited);
    const editedRecordId = recordBeingEdited?.id ?? null;
    ...
    /*
     * v26:
     * DB更新自体は成功していても、直後の再取得でブラウザ側に
     * 古い一覧が残るケースがあったため、再取得後に今回保存した
     * payloadをローカル一覧にも確実に反映して再描画します。
     */
    await loadRecords(false);

    if (wasEditing && editedRecordId !== null) {
      const index = allRecords.findIndex(
        (record) => String(record.id) === String(editedRecordId)
      );
      ...
      if (
        currentDetailRecord &&
        String(currentDetailRecord.id) === String(editedRecordId)
      ) {
        currentDetailRecord = {
          ...currentDetailRecord,
          ...payload
    ...
    setMessage(
      addMessage,
      `${recordBeingEdited ? "更新" : "保存"}できませんでした：${
        error?.message || "不明なエラー"
      }`,
      "error"
    );
...
function createFishCard(record) {
  ...
  editRow.className = "fish-card-edit-row";

  const editButton = document.createElement("button");
  editButton.type = "button";
  editButton.className = "fish-card-edit-button";
  editButton.textContent = "編集";
  editButton.setAttribute(
    "aria-label",
    `${record.fish_name}を編集`
  );
  ...
  if (record.fish_name === "不明魚") {
    const aiButton = document.createElement("button");
    aiButton.type = "button";
    aiButton.className = "fish-card-ai-button";
    aiButton.textContent = "AI判定";
    ...
    aiButton.addEventListener("click", (event) => {
      event.stopPropagation();
      openEditModalAndIdentify(record);
    });

    editRow.appendChild(aiButton);
  }
...
aiFishButton.addEventListener("click", () => {
  identifyFishWithAi(editingRecord);
});
...
detailEditRecordButton.addEventListener(
  "click",
  editCurrentRecord
);
