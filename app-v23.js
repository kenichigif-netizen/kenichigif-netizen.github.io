console.log("fish-memo AI fish identification v23");
...
const FISH_AI_FUNCTION_NAME = "fish-ai";
...
async function identifyFishWithAi(record = editingRecord) {
  ...
  const selectedBlob = selectedImageBlobs[0] || null;
  const existingImagePath =
    record?.image_path || record?.image_path_2 || null;

  if (!selectedBlob && !existingImagePath) {
    setMessage(
      aiFishMessage,
  ...
  setMessage(
    aiFishMessage,
    "写真をAIで確認しています。数秒〜十数秒かかる場合があります…",
    "info"
  );
  ...
  try {
    ...
    if (selectedBlob) {
      body.imageData = await blobToBase64(selectedBlob);
      body.mimeType = selectedBlob.type || "image/jpeg";
    } else {
    ...
    const response = await withTimeout(
      fetch(
        `${SUPABASE_URL}/functions/v1/${FISH_AI_FUNCTION_NAME}`,
        {
          method: "POST",
          headers: {
    ...
    if (!response.ok) {
      ...
      if (response.status === 404) {
        throw new Error(
          "AI判定用のSupabase Edge Function「fish-ai」がまだ公開されていません。"
        );
      }
    ...
    console.error("fish AI identification error", error);
    setMessage(
      aiFishMessage,
      `AI判定できませんでした：${
        error?.message || "不明なエラー"
